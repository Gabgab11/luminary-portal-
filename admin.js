...
// A second, independent Firebase app instance — used ONLY to create a new
// client's Firebase Auth account. Auth state is per-app-instance, so doing
// this on a secondary app means creating (and then signing out of) their
// account never touches your own signed-in admin session on the primary
// `auth` instance above.
const secondaryApp = initializeApp(firebaseConfig, "ClientAccountCreator");
...
const linkResultNote = document.getElementById("linkResultNote");
...
/* ------------------------------------------------------------------ */
/* Create a new client + their real login + generate the portal link   */
/* ------------------------------------------------------------------ */
...
function randomTempPassword() {
  // Never shown to anyone or stored — createUserWithEmailAndPassword just
  // needs *some* password to stand the account up. The client sets their
  // real one via the password-reset email sent right after.
  if (window.crypto && crypto.randomUUID) return crypto.randomUUID();
  return Math.random().toString(36).slice(2) + Date.now().toString(36);
}

// Creates the client's Firebase Auth account on the secondary app instance
// (so your own admin session is untouched), then emails them a link to set
// their own password. Returns { created: true } for a brand-new account, or
// { created: false } if that email already had an account — in that case we
// still send them a reset email so they can get in.
async function createClientLogin(email) {
  try {
    await createUserWithEmailAndPassword(secondaryAuth, email, randomTempPassword());
...
createClientBtn.addEventListener("click", async () => {
  ...
  if (!email || !email.includes("@")) {
    alert("Enter the client's email so we can set up their login.");
    return;
  }
  ...
  try {
    const { created } = await createClientLogin(email);
    ...
    linkResultNote.textContent = created
      ? `We've emailed ${email} a link to set their password and sign in.`
      : `${email} already had a login — we sent them a password reset link instead.`;
    ...
    if (err.code === "auth/invalid-email") {
      alert("That email address doesn't look valid — double-check it.");
    } else {
...
function listenToClients() {
  ...
  onSnapshot(clientsRef, (snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      const emailLabel = c.clientEmail ? ` · ${c.clientEmail}` : "";
      ...
      row.innerHTML = `
        <div class="client-main">
          ...
          <span class="client-meta">${durationLabel}${startLabel}${emailLabel}</span>
